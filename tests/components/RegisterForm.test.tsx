...
describe("RegisterForm", () => {
  ...
  it("nie wyświetla błędów walidacji, gdy dane są poprawne", async () => {
    ...
    await user.type(screen.getByLabelText("Hasło logowania"), "Password123!");
    await user.click(screen.getByRole("checkbox", { name: /Akceptuję regulamin i politykę prywatności/i }));
    await user.click(screen.getByRole("button", { name: "Utwórz konto" }));
    ...
    expect(screen.queryByText(/Musisz zaakceptować regulamin i politykę prywatności/)).not.toBeInTheDocument();
    ...
    expect(screen.getByRole("checkbox", { name: /Akceptuję regulamin i politykę prywatności/i })).not.toHaveAttribute("aria-invalid", "true");
  });
